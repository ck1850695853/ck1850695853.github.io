...
//根据前端参数插入数据
$sql = "SELECT * FROM `shops` WHERE `product_id`='$id'";
...
if($rows>0){
	...
	$sql = "UPDATE `shops` SET `product_num`='$num' WHERE `product_id`='$id'";
}else{
	$sql = "INSERT INTO `shops` (`product_id`,`product_img`,`product_name`,`product_num`,`product_price`) VALUES ('$id','$img','$name','$num','$price')";
}
